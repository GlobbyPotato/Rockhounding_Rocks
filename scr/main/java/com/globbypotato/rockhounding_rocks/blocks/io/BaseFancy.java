...
public class BaseFancy extends DecoIO{
    public static final PropertyEnum POS = PropertyEnum.create("pos", EnumPos.class);

	public BaseFancy(String name, Material material, String[] array, float hardness, float resistance, SoundType stepSound) {
	...
	private static EnumPos columnConnections(IBlockAccess worldIn, BlockPos pos) {
        ...
        if(multi){
	        if(stateUP.getBlock() instanceof BaseFancy && stateDOWN.getBlock() instanceof BaseFancy){
	        	return EnumPos.TORSO;
	        }else if(!(stateUP.getBlock() instanceof BaseFancy) && stateDOWN.getBlock() instanceof BaseFancy){
	        	return EnumPos.TOP;
	        }else if(stateUP.getBlock() instanceof BaseFancy && !(stateDOWN.getBlock() instanceof BaseFancy)){
