...
public class BaseColumns extends DecoIO {
    public static final PropertyEnum POS = PropertyEnum.create("pos", EnumPos.class);

	public BaseColumns(String name, Material material, String[] array, float hardness, float resistance, SoundType stepSound) {
	...
	private static EnumPos columnConnections(IBlockAccess worldIn, BlockPos pos) {
        ...
        if(multi){
	        if(stateUP.getBlock() instanceof BaseColumns && stateDOWN.getBlock() instanceof BaseColumns){
	        	return EnumPos.TORSO;
	        }else if(!(stateUP.getBlock() instanceof BaseColumns) && stateDOWN.getBlock() instanceof BaseColumns){
	        	return EnumPos.TOP;
	        }else if(stateUP.getBlock() instanceof BaseColumns && !(stateDOWN.getBlock() instanceof BaseColumns)){
