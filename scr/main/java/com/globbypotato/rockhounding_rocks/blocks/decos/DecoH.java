...
public class DecoH extends DecoIO {
	public static final PropertyEnum VARIANT = PropertyEnum.create("type", EnumRocksH.class);

    public DecoH(String[] array, String name){
        super(name, Material.ROCK, array, 3.0F, 5.0F, SoundType.STONE);
		for(int x = 0; x < 16; x++){setHarvestLevel(EnumRocksA.values()[x].tool(), EnumRocksH.values()[x].harvest(), getState(x));}
