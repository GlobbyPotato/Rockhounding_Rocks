...
public class DecoG extends DecoIO {
	public static final PropertyEnum VARIANT = PropertyEnum.create("type", EnumRocksG.class);

    public DecoG(String[] array, String name){
        super(name, Material.ROCK, array, 3.0F, 5.0F, SoundType.STONE);
		for(int x = 0; x < 16; x++){setHarvestLevel(EnumRocksA.values()[x].tool(), EnumRocksG.values()[x].harvest(), getState(x));}
