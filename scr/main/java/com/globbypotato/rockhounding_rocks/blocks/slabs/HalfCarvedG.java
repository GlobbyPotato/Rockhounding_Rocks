...
public class HalfCarvedG extends SlabsIO {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ){
		...
		if(holdingSlabLo(heldItem, ModBlocks.HALF_CARVED_G_LO) && isDoublingSlab(world, pos, heldItem, side, ModBlocks.HALF_CARVED_G_LO, ModBlocks.HALF_CARVED_G_HI)){
			world.setBlockState(pos, ModBlocks.CARVED_G.getStateFromMeta(heldItem.getItemDamage()));
    ...
    public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player){
        return new ItemStack(ModBlocks.HALF_CARVED_G_LO, 1, this.damageDropped(state));
	}

    @Nullable
	@Override
    public Item getItemDropped(IBlockState state, Random rand, int fortune){
        return Item.getItemFromBlock(ModBlocks.HALF_CARVED_G_LO);
    }
