...
public class HalfPolishedB extends SlabsIO {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ){
		...
		if(holdingSlabLo(heldItem, ModBlocks.HALF_POLISHED_B_LO) && isDoublingSlab(world, pos, heldItem, side, ModBlocks.HALF_POLISHED_B_LO, ModBlocks.HALF_POLISHED_B_HI)){
			world.setBlockState(pos, ModBlocks.POLISHED_B.getStateFromMeta(heldItem.getItemDamage()));
    ...
    public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player){
        return new ItemStack(ModBlocks.HALF_POLISHED_B_LO, 1, this.damageDropped(state));
	}

    @Nullable
	@Override
    public Item getItemDropped(IBlockState state, Random rand, int fortune){
        return Item.getItemFromBlock(ModBlocks.HALF_POLISHED_B_LO);
    }
